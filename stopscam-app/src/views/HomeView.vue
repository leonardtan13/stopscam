      ...
      <div class="container w-full mx-auto">
          <div v-if="isScam" class="transColor w-3/4 mx-auto p-6 rounded-xl">
            ...
            <p class="text-lg text-center mt-10"> 
              <a class="font-bold" href="/register">Join</a> our community to help us spread words.
            </p>
          </div>
          ...
          <div v-else-if="isSimilar" class="transColor w-3/4 mx-auto p-6 rounded-xl">
            <div class="grid-flow-row auto-rows-max justify-items-center">
              ...
              <div>
                ...
                <ul>
                  <li v-for="post of similarityResults.posts" class="text-lg leading-10 ml-10">
                    <span class="font-semibold">{{(post.rating*100).toFixed(2)}}%</span> 
                    similar to 
                    <span class="italic">{{post.target}}</span> 
                    <span v-if="store.posts.get(post.id).isLegitSite == true" class="text-white bg-emerald-700 font-light text-sm ml-2 p-2 rounded-md ">Legit</span>
                    <span v-else class="text-white bg-rose-700  font-light text-sm ml-2 p-2 rounded-md">Scam</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div v-else-if="isEmpty" class="transColor w-3/4 mx-auto p-6 rounded-xl">
            <p class="text-3xl text-center my-4"><span class="italic">Whoops</span>, there's nothing in there.</p>
